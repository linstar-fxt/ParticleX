...
public class ParticleXCommand {
    ...
    public static int execute(ServerCommandSource source, ParticleEffect particleEffect , Vec3f color, Vec3f target, Vec3d pos, Vec3d delta, float speed, int count, boolean force, Collection<ServerPlayerEntity> viewers, int age, float alpha, float scale, int delay, int color_delay){
        ...
        CustomParticlePacket packet = new CustomParticlePacket(particleEffect, color.getX(), color.getY(), color.getZ(), target.getX(), target.getY(), target.getZ(), pos.x, pos.y, pos.z, delta.x, delta.y, delta.z, speed, count, force, age, alpha, scale, delay, color_delay);
